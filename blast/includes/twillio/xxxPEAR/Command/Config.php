 ...
 * base class
 */
require_once 'Command/Common.php';
...
',
    ...
    function doConfigCreate($command, $options, $params)
    {
        ...
        $config->noRegistry();
        $config->set('php_dir', $windows ? "$root\\pear\\php" : "$root/php", 'user');
        $config->set('data_dir', $windows ? "$root\\pear\\data" : "$root/data");
        $config->set('www_dir', $windows ? "$root\\pear\\www" : "$root/www");
        $config->set('cfg_dir', $windows ? "$root\\pear\\cfg" : "$root/cfg");
        $config->set('ext_dir', $windows ? "$root\\pear\\ext" : "$root/ext");
        $config->set('doc_dir', $windows ? "$root\\pear\\docs" : "$root/docs");
        $config->set('test_dir', $windows ? "$root\\pear\\tests" : "$root/tests");
        $config->set('cache_dir', $windows ? "$root\\pear\\cache" : "$root/cache");
        $config->set('download_dir', $windows ? "$root\\pear\\download" : "$root/download");
        $config->set('temp_dir', $windows ? "$root\\pear\\temp" : "$root/temp");
